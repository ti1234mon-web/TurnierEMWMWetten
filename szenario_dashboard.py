...
# ============================================================
# FINAL VERSION: ALL-IN-ONE DASHBOARD
# MIT ALLEN 48 WM-TEILNEHMERN 2026
# 9 KPIS, FIFA-RANKING, HISTORISCHEN DURCHSCHNITTSTOREN,
# DYNAMISCHER BEGRENZUNG, SDM, SPIELSTIL, MODULATION, POISSON
# ============================================================
...
# ============================================================
# 1.1 FIFA-RANKING (ALLE 48 WM-TEILNEHMER 2026)
# ============================================================
...
FIFA_RANKINGS = {
    ...
    # CONCACAF (Nord- & Mittelamerika) – 6 Teams
    "USA": 14,
    "Kanada": 30,
    "Mexiko": 15,
    "Costa Rica": 45,
    ...
    # Zusätzliche (für Vollständigkeit)
    "Island": 59,
    "Slowakei": 60,
    "Slowenien": 61,
    "Nordirland": 62,
...
# ============================================================
# 1.2 HISTORISCHE DURCHSCHNITSTORE (ALLE TEAMS)
# ============================================================
...
def get_historical_avg(team_name: str) -> float:
    return HISTORICAL_AVG.get(team_name, 1.2)
...
def get_lambda_from_expected(expected_goals: float) -> float:
    if expected_goals <= 0:
        return 0.0
    return min(max(expected_goals, 0.0), 5.0)
...
def apply_strength_difference_modulator(lambda_a: float, lambda_b: float) -> Tuple[float, float]:
    ...
    sdm_factor = 1.0 + (strong_lambda / 5.0) * 1.0

    strong_goals = strong_lambda + (diff * sdm_factor * 0.3)
    weak_goals = max(0.1, weak_lambda - (diff * sdm_factor * 0.2))
...
class TeamScenario:
    ...
    def calculate_expected_goals(self) -> float:
        ...
        hist_avg = get_historical_avg(self.team_name)
        ...
        if self.kpis["Offensive"] == "Stark" and self.kpis["Defensive"] == "Schwach":
            max_addition += 0.3

        max_allowed = hist_avg + max_addition
        final_expected = min(expected, max_allowed)

        return max(0.0, min(final_expected, 5.0))
        ...
        st.markdown(f"""
            <div style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); border-radius: 16px; padding: 1rem; text-align: center;">
                ...
                <div style="color: rgba(255,255,255,0.3); font-size: 0.6rem; margin-top: 0.3rem;">
                    Hist. Schnitt: {get_historical_avg(match.team_a.team_name):.1f} / {get_historical_avg(match.team_b.team_name):.1f}
                </div>
            </div>
        """, unsafe_allow_html=True)
...
st.sidebar.markdown("### 📖 Die 10 Schritte")
st.sidebar.markdown("""
1. **Team** – wähle ein Team
...
6. **Historischer Schnitt** – pro Team (0.5–2.8)
7. **Stärke-Anpassung** – manuell (±0.8)
8. **Form-Faktor** – aus KPI (0.5–1.5)
9. **Spielstil-Modulator** – defensiv+tief → ×0.5, aggressiv+hoch → ×1.3
10. **SDM** – Stärke-Differenz-Modulator
11. **Poisson** – Wahrscheinlichkeiten für Ergebnisse
""")
st.sidebar.markdown("---")
st.sidebar.caption("📅 Szenario Dashboard v7.0 (mit allen WM-Teams)")
...
with col2:
    team_b, kpis_b = render_team_input("🔵 Team B", "b", default_team="Elfenbeinküste")
